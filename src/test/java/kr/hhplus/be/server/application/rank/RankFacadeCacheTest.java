...
import kr.hhplus.be.server.infra.cache.RedisCacheTemplate;
import kr.hhplus.be.server.supporters.cache.CacheType;
import kr.hhplus.be.server.test.supporters.IntegrationTestSupport;
import kr.hhplus.be.server.test.supporters.db.RedisCacheCleaner;
...
class RankFacadeCacheTest extends IntegrationTestSupport {
    ...
    @BeforeEach
    void setUp() {
        product1 = Product.create("상품명1", 1_000L, SellingStatus.SELLING);
        product2 = Product.create("상품명2", 2_000L, SellingStatus.SELLING);
        product3 = Product.create("상품명3", 3_000L, SellingStatus.STOP_SELLING);
