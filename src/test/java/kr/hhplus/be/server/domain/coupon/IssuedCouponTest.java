...
class IssuedCouponTest {
    ...
    void createIssuedCoupon_success() {
        Coupon coupon = Coupon.of(
                "10% 할인", DiscountType.PERCENTAGE,
                BigDecimal.valueOf(10), 100,
                LocalDateTime.now().minusDays(1),
                LocalDateTime.now().plusDays(3)
        );

        IssuedCoupon issued = IssuedCoupon.of(coupon, 999L);

        assertThat(issued.getUserId()).isEqualTo(999L);
        assertThat(issued.getCouponName()).isEqualTo(coupon.getName());
    ...
    void useCoupon_success() {
        Coupon coupon = Coupon.of(
                "5,000원 할인", DiscountType.FIXED_AMOUNT,
    ...
    void useCoupon_fail_alreadyUsed() {
        Coupon coupon = Coupon.of(
                "한번만 쿠폰", DiscountType.FIXED_AMOUNT,
                BigDecimal.valueOf(1000), 100,
                LocalDateTime.now().minusDays(1), LocalDateTime.now().plusDays(1)
        );
        IssuedCoupon issued = IssuedCoupon.of(coupon, 123L);
        issued.use();
    ...
    void isValidNow_success() {
        Coupon coupon = Coupon.of(
                "기간 내 쿠폰", DiscountType.PERCENTAGE,
                BigDecimal.valueOf(5), 10,
                LocalDateTime.now().minusDays(1), LocalDateTime.now().plusDays(1)
        );
        IssuedCoupon issued = IssuedCoupon.of(coupon, 1L);

        assertThat(issued.isValidNow(LocalDateTime.now())).isTrue();
    }
    ...
    void isValidNow_fail_by_coupon() {
        Coupon coupon = Coupon.of(
                "기간 지난 쿠폰", DiscountType.FIXED_AMOUNT,
                BigDecimal.valueOf(5000), 10,
                LocalDateTime.now().minusDays(10), LocalDateTime.now().minusDays(5)
        );
        IssuedCoupon issued = IssuedCoupon.of(coupon, 1L);

        assertThat(issued.isValidNow(LocalDateTime.now())).isFalse();
    }
}
